...
void testCreate() {
	...
	SortedIndexedListIterator it = list.iterator();
	assert(!it.valid());
	it.first();
...
void testDeleteSearch(Relation r) {
	...
	SortedIndexedListIterator it1 = list.iterator();
	assert(it1.valid());
	it1.first();
	assert(it1.valid());
	TComp deleted = NullTComp;
	try {
		...
		it1.first();
		SortedIndexedListIterator it2 = list.iterator();
		assert(it2.valid());
		it2.first();
		assert(it2.valid());
