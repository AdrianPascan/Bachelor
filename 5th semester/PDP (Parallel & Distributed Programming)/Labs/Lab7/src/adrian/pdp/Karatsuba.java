...
public class Karatsuba {
    ...
    public static int[] polyMulRec(int size, int[] first, int[] second, int mpiMe, int mpiSize, int power) {
        //MPI destinations
        int coefficient = (int) Math.pow(3, power);
        ...
        if (firstDest < mpiSize) {
            ...
            MPI.COMM_WORLD.Send(secondUpper, 0, middle, MPI.INT, firstDest, 0);
        }
        else {
            firstMul = polyMulRec(middle, firstUpper, secondUpper, firstDest, mpiSize, power + 1);
        }
        if (secondDest < mpiSize) {
            ...
            MPI.COMM_WORLD.Send(secondLower, 0, middle, MPI.INT, secondDest, 0);
        }
        else {
            thirdMul = polyMulRec(middle, firstLower, secondLower, secondDest, mpiSize, power + 1);
        }
        int[] secondMul = polyMulRec(
                middle,
                Common.addArrays(firstLower, firstUpper),
                Common.addArrays(secondLower, secondUpper),
                mpiMe,
                mpiSize,
                power + 1
        );
        ...
        if (firstDest < mpiSize) {
            ...
            firstMul = new int[mulSize[0]];
            MPI.COMM_WORLD.Recv(firstMul, 0, mulSize[0], MPI.INT, firstDest, MPI.ANY_TAG);
        }
        if (secondDest < mpiSize) {
            int[] mulSize = new int[1];
        ...
        int[] secondTerm = Common.addToArrayAtTheBeginning(
                Common.subtractArrays(
                        Common.subtractArrays(secondMul, firstMul),
                        thirdMul
                ),
                0,
                middle
        );
        int[] thirdTerm = thirdMul;
